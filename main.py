...
from config import BOT_TOKEN, DB_PATH, get_timezone
...
async def add_transaction_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None: 
    ...
    # await update.message.reply_text(
    #     f"Saved: {parsed['type']} {parsed['amount']:.2f} in '{parsed['category']}'."
    # )


async def daily_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await _report_command(update, context, "daily")
...
async def _report_command(
    ...
    report = build_report(database, update.effective_user.id, period, _now(tz))
...
def main() -> None:
    ...
    application.add_handler(
        MessageHandler(filters.TEXT & ~filters.COMMAND, add_transaction_message)
    )

    application.post_init = set_commands
    setup_scheduler(application, database, tz)
